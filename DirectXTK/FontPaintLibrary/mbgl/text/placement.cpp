...
void getSegmentGlyphs(std::back_insert_iterator<GlyphInstances> glyphs, Anchor &anchor,
    ...
    while (true) {
        ...
        float angle =
            -std::atan2(end.x - newAnchor.x, end.y - newAnchor.y) + direction * DirectX::XM_PI / 2.0f;
        if (upsideDown)
            angle += DirectX::XM_PI;

        // Don't place around sharp corners
        float angleDiff = std::fmod((angle - prevAngle), (2.0f * DirectX::XM_PI));
        ...
        glyphs = GlyphInstance{
            /* anchor */ newAnchor,
            /* offset */ static_cast<float>(upsideDown ? DirectX::XM_PI : 0.0),
            /* minScale */ scale,
            /* maxScale */ prevscale,
            /* angle */ static_cast<float>(std::fmod((angle + 2.0 * DirectX::XM_PI), (2.0 * DirectX::XM_PI)))};
...
Placement Placement::getIcon(Anchor &anchor, const Rect<uint16_t> &image, float boxScale,
    ...
    float angle = props.icon.rotate * DirectX::XM_PI / 180.0f;
    if (anchor.segment >= 0 && props.icon.rotation_alignment != RotationAlignmentType::Viewport) {
        ...
        angle += -std::atan2(next.x - anchor.x, next.y - anchor.y) + DirectX::XM_PI / 2;
    }
...
Placement Placement::getGlyphs(Anchor &anchor, const vec2<float> &origin, const Shaping &shaping,
    ...
    const float maxAngle = props.text.max_angle * DirectX::XM_PI / 180;
    const float rotate = props.text.rotate * DirectX::XM_PI / 180;
    ...
    for (const PositionedGlyph &shape : shaping) {
        ...
        for (const GlyphInstance &instance : glyphInstances) {
            ...
            placement.shapes.emplace_back(
                tl, tr, bl, br, rect,
                float(std::fmod((anchor.angle + rotate + instance.offset + 2 * DirectX::XM_PI), (2 * DirectX::XM_PI))),
