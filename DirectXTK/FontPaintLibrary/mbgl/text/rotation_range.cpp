...
rotatingRotatingCollisions(const CollisionRect &a, const CollisionRect &b,
    ...
    const std::array<float, 8> c = {{
        ...
        /*[1]*/ static_cast<float>(std::asin((float)(a.br.y - b.tl.y) / d) + DirectX::XM_PI),
        /*[2]*/ static_cast<float>(2 * DirectX::XM_PI -
                                   std::asin((float)(-a.tl.y + b.br.y) / d)),
        /*[3]*/ static_cast<float>(DirectX::XM_PI - std::asin((float)(-a.tl.y + b.br.y) / d)),

        // left/right
        /*[4]*/ static_cast<float>(2 * DirectX::XM_PI -
                                   std::acos((float)(a.br.x - b.tl.x) / d)),
        /*[5]*/ static_cast<float>(std::acos((float)(a.br.x - b.tl.x) / d)),
        /*[6]*/ static_cast<float>(DirectX::XM_PI - std::acos((float)(-a.tl.x + b.br.x) / d)),
        /*[7]*/ static_cast<float>(DirectX::XM_PI +
    ...
    for (size_t i = 0; i < c.size(); i++) {
        ...
        if (!std::isnan(c[i]) && d_sq <= e[i]) {
            ...
            f.push_back(
                std::fmod((c[i] + angleBetweenAnchors + 2 * DirectX::XM_PI), (2 * DirectX::XM_PI)));
        }
    }
