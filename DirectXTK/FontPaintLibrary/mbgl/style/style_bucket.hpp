...
public:
	...
	inline StyleBucketSymbol(StyleBucketSymbol && src) { min_distance = src.min_distance; avoid_edges = src.avoid_edges; icon = std::move(src.icon); placement = src.placement; text = std::move(src.text); }
	inline StyleBucketSymbol& operator=(StyleBucketSymbol && src){
		if (this != &src)
		{
			min_distance = src.min_distance; avoid_edges = src.avoid_edges; icon = std::move(src.icon); placement = src.placement; text = std::move(src.text);
		}
		return *this;
	}
